...
void printStatusSummary() {
  ...
  // GPS status
  Serial.print(F("GPS: "));
  if (gpsStatus.isWorking) {
    Serial.print(F("OK - Fix: "));
    Serial.print(GPS_fixType);
    Serial.print(F(", Sats: "));
    Serial.print(SIV);
    Serial.print(F(", pDOP: "));
    Serial.println(pDOP / 100.0);
  } else {
  ...
  if (flightState >= BOOST) {
    ...
    if (boostEndTime > 0) {
      ...
      Serial.print((boostEndTime - flightStartTime) / 1000.0);
      Serial.println(F(" sec"));
    }
    ...
    if (flightState >= APOGEE) {
      ...
      if (baroCalibrated) {
        Serial.print(F("AGL: "));
        Serial.print(maxAltitudeReached - launchAltitude);
        Serial.println(F(" m"));
      }
    }
  }
  ...
  Serial.println(watchdogTriggered ? F("TRIGGERED") : F("OK"));
