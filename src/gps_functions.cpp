...
void gps_init() {
  ...
  while (attempts < maxAttempts) {
    ...
    if (myGNSS.begin(Wire) == true) {
      Serial.println(F("GPS module started!"));
      break;
...
void gps_print(){
    ...
    if (GPS_fixType >= 2) {
      ...
      Serial.print(F(" | RTK: "));
      Serial.print(RTK);
      if (RTK == 1) Serial.print(F(" - Float RTK solution"));
      else if (RTK == 2) Serial.print(F(" - Fixed RTK solution"));
      else Serial.print(F(" - No RTK solution"));
