...
bool loadStateFromEEPROM() {
  ...
  if (stateData.signature != EEPROM_SIGNATURE_VALUE) {
    if (enableSystemDebug) { // Only print if debug is enabled
...
void recoverFromPowerLoss() {
  ...
  switch (savedStateEnum) {
    ...
    case PAD_IDLE:
      ...
      if (enableSystemDebug) Serial.println(F("Recovery: Resetting to STARTUP."));
      break;
    ...
    case COAST:
      ...
      if (enableSystemDebug) Serial.println(F("Recovery: Was BOOST/COAST, transitioning to DROGUE_DESCENT. Max alt restored."));
      break;
    ...
    case DROGUE_DEPLOY:
      ...
      if (enableSystemDebug) Serial.println(F("Recovery: Was APOGEE/DROGUE_DEPLOY, ensuring DROGUE_DEPLOY state."));
      break;
    ...
    case DROGUE_DESCENT:
      ...
      if (enableSystemDebug) Serial.println(F("Recovery: Resuming DROGUE_DESCENT. Altitudes restored."));
      break;

    case MAIN_DEPLOY:
      // Similar to drogue, ensure main deployment logic is triggered.
      currentFlightState = MAIN_DEPLOY;
      if (enableSystemDebug) Serial.println(F("Recovery: Was MAIN_DEPLOY, ensuring MAIN_DEPLOY state."));
      break;
