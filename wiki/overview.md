...
Flight computer firmware for a guided amateur rocket on Teensy 4.1 (ARM Cortex-M7). Handles sensor fusion, 14-state flight state machine, guidance/control, data logging, and post-flight recovery. Current version: **v0.10.0** (Safety Features phase complete; note the boot banner still prints the older `TRIPLET_FLIGHT_VERSION` 0.51 from the main file).
...
├── telemetry.cpp/.h              # 40-byte packet + 43-byte framed Serial5 downlink (behind ENABLE_TELEMETRY)
├── sensor_validator.h            # Cross-sensor sanity checks (dormant — never instantiated)
├── utility_functions.cpp/.h      # Shared math, hypsometric formula
├── apogee_detector.h             # 2-of-3 apogee voting class (dormant — live detectApogee() is in flight_logic.cpp)
├── watchdog_recovery.h           # Watchdog reset recovery helpers (dormant — never included)
...
├── hal/                          # Hardware Abstraction Layer (Phase 1 — dormant, not wired into flight build)
...
└── sensors/                      # Sensor Modularity Layer (Phase 2 — dormant, not wired into flight build)
...
test/
├── test_state_machine/           # Unity suites (one folder per suite — PlatformIO layout, 12 suites)
├── test_flight_logic/
...
```
Sensors (polled at 100ms / 10 Hz) → Kalman Filter (AHRS, ~10 Hz) → Flight Logic → Guidance/PID (50 Hz)
       ↓                                                                  ↓
  Data Logging (CSV on SD, 5 Hz) ←────── LogData struct ──────── Actuators (servos, pyros)
       ↓
...
1. **Layered architecture** — HAL → sensors → flight logic → guidance. *Design intent; the flight build currently bypasses layers 0–1 and calls Arduino APIs / C drivers directly ([[concepts/layered-architecture]])*
2. **HAL abstraction** — implemented for desktop unit testing without Teensy hardware; **not yet wired into the flight build** (dormant scaffolding)
3. **IMUInterface adapters** — runtime polymorphism for sensor swap/failover; **dormant** — the only live failover is the inline Kalman accel-source switch to the KX134 above 16 g
4. **2-of-3 apogee voting** — designed (class exists in `src/apogee_detector.h`) but dormant; the live `detectApogee()` fires on the **first match** of baro / accel / GPS descent or the 20 s backup timer
5. **EEPROM state persistence** — survives power-loss and watchdog resets; operates on the live state globals and also persists the baro calibration offset
6. **Kalman filter** — replaces deprecated Madgwick; 3-state scalar filter over gyro + accel + mag (Euler state; quaternion migration planned — see [[concepts/kalman-filter]])
...
Development is organised into phases tracked against semantic versions. Status as of 2026-07-02 (note: Phases 1-2 delivered the HAL / sensor-abstraction *code*, but wiring it into the flight build is still outstanding — see [[concepts/layered-architecture]]):

| Phase | Version target | Scope | Status |
|-------|---------------|-------|--------|
| 1 HAL foundation | v0.7.0 | 8 HAL interfaces, Teensy + Mock | ✅ merged (dormant in flight build) |
| 2 Sensor modularity | v0.8.0 | `IMUInterface`, adapters, `IMUManager` | ✅ merged (dormant in flight build) |
...
| 5 Documentation & polish | v1.0.0-rc1 | wiki consolidation (single source of truth) | ✅ merged |
| 6 Advanced features + production readiness | v1.0.0 | Trajectory, power/thermal, pre-flight, validation | 🟡 6.2 merged; 6.1 in progress; 6.3 deferred to v1.1; 6.4 follows 6.1 |
